...
- [x] **10 — `@asnewyla/tokens`** · Color/spacing/radius/typography scales as CSS custom properties under `--xd-*`, values copied 1:1 from `Button.css`'s existing hardcoded fallbacks (colors incl. dark-mode overrides; spacing/radius/font-size/weight/line-height newly named as a scale). Spacing uses `xs`/`sm`/`md`/`lg`/`xl`/`2xl`, not numeric indices — matches the radius scale's naming, changed after user review (numeric names carry no size information on their own). No JS — `tsup` bundles the bare `src/tokens.css` entry directly (tsup supports CSS-only entries; no dummy JS import needed). `Button.css` gained a sync-note comment pointing at the package as canonical, no code change (soft dependency was already the pattern). Storybook: `Foundations/Tokens` docs page (Colors/Spacing/Radius/Typography stories) + `preview.tsx` now imports `@asnewyla/tokens/tokens.css` globally. Verified with Playwright screenshots (installed for this) of every existing Button/IconButton/ButtonGroup story — pixel-identical to before, zero console errors, confirming the soft-dependency swap is a true no-op.
...
- Module 10 (`@asnewyla/tokens`) built via TDD (5 tests) on `feature/tokens`, not yet merged — see below.

**In progress**
- Nothing on the Button family — Modules 1–8 complete, merged into `main`, and actually live (both Storybook and the npm packages).
- Scope expanded 2026-08-20: Modules 9–14 planned (Changesets, `tokens`, `image`, `layout`, `input`, `form`). Plan reviewed same day (see Architecture Decisions for the resulting rows). Module 9 merged; Module 10 done on `feature/tokens`, pending commit/merge.

**Next**
- Commit and merge `feature/tokens` (Module 10).
- Module 11: `@asnewyla/image`.
