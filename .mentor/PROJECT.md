...
- [ ] **10 — `@asnewyla/tokens`** · Design tokens package (renamed from `styleguide`, see Architecture Decisions): color, spacing, typography, radius scales as CSS custom properties under the existing `--xd-*` prefix, shipped as an optional stylesheet (soft dependency — see Tokens dependency model). Migrate `Button.css`'s hardcoded fallback values to reference this as the source of truth (verify via existing Storybook + test suite — no visual regression). Plus Storybook stories/docs page for the token set.
- [ ] **11 — `@asnewyla/image`** · Non-interactive (no hover/press/focus state), no primitive split — still has internal state for the `fallback`/`onError` handling. Mandatory `alt` prop (TS-required, forces a deliberate a11y choice — pass `alt=""` explicitly for decorative images rather than a silent default). Props: `src`, `alt`, `aspectRatio`, `fit` (object-fit), `radius` (token-based), `fallback` (on load error). Plus Storybook stories.
...
- Scope expanded 2026-08-20: Modules 9–14 planned (Changesets, `tokens`, `image`, `layout`, `input`, `form`). Plan reviewed same day (see Architecture Decisions for the resulting rows). Module 9 done same day (not yet committed/merged — see below).

**Next**
- Commit and merge Module 9 (Changesets setup) — currently done on disk, not yet in git history.
- Module 10: `@asnewyla/tokens` — new package, no existing packages to retrofit yet, so this is a clean scaffold-and-build rather than a migration. `Button.css`'s hardcoded fallbacks get migrated to reference it afterward, once the token values exist to migrate *to*.
